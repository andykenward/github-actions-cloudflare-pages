...
- **Versions**: Node via `engines`, pnpm via `packageManager` — both in [package.json](package.json). `packageManager` is the one place to bump pnpm: CI's `pnpm/action-setup` reads it directly, [bin/sync-versions.ts](bin/sync-versions.ts) (the first step of `pnpm run all`) copies it to the devcontainer's `pnpmVersion`, and [`__tests__/scripts/sync-versions.test.ts`](__tests__/scripts/sync-versions.test.ts) fails CI when they drift. Dependabot doesn't bump `packageManager`; respect `minimumReleaseAge` in [pnpm-workspace.yaml](pnpm-workspace.yaml) when choosing a version.
