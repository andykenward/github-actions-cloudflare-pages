...
- **Never bump `undici` across a major.** It is pinned to Node's bundled major on purpose ([.github/dependabot.yml](.github/dependabot.yml) ignores its majors). `setGlobalDispatcher()` patches the dispatcher that Node's **built-in** `fetch` reads, and the two sides find each other through a versioned global symbol (`Symbol.for('undici.globalDispatcher.N')`). undici 8 bumped that symbol to `.2` while Node 24 still reads `.1`, so on v8 `MockAgent` **silently stops intercepting** and tests hit the real network — they fail with genuine API responses, not connection errors. Check `process.versions.undici` before touching the version; only raise the major once Node's bundled major moves.
...
- **TypeScript 6 + 7 side by side**: TS 7 is the Go-native compiler and ships **no programmatic API** until 7.1, so [package.json](package.json) installs both under npm aliases — `"@typescript/native": "npm:typescript@7"` supplies the `tsc` binary (what `tsc:check` / `tsc:ls` / CI run), and `"typescript": "npm:@typescript/typescript6@6"` keeps the 6.0 API on the `typescript` module specifier for the tools that `import ts from 'typescript'` (`openapi-typescript` in `codegen:cloudflare`, and `cosmiconfig` via `graphql-config`, which loads [graphql.config.ts](graphql.config.ts)) plus a `tsc6` binary. Bump the two independently, and never collapse them back to a single `typescript` dependency until 7.1 ships the API. `knip` needs neither (it parses with `oxc-parser`), and `oxlint-tsgolint` bundles its own binary.
- **`@effect/tsgo`**: the Effect language service is delivered by patching binaries in place, not by a separate LSP process. The `prepare` script runs `effect-tsgo patch --typescript --oxlint` on every install, patching the platform `tsc` under `@typescript/native` and the `oxlint` / `oxlint-tsgolint` binaries (`effect-tsgo unpatch` reverses it). **Effect diagnostics are reported by `pnpm run lint`, not `tsc:check`** — the `@effect/language-service` plugin in [tsconfig.json](tsconfig.json) is registered with `"diagnostics": false` so the two passes don't double-report, per [the Effect devtools docs](https://effect.website/docs/v4/getting-started/devtools#oxlint). [.oxlintrc.json](.oxlintrc.json) extends the `correctness` + `antipattern` presets from `node_modules/@effect/tsgo/oxlint-presets/` and adds `effecttsgo` to `plugins`; the `effect-native` and `style` presets (and so `recommended`, which contains them) are deliberately left out — they flag every `async` function, `process.env` read and `node:*` import in this non-Effect codebase (140 warnings). Preset rules land as `warn`, and `lint` has no `--max-warnings`, so they do not fail CI. That plugin name is not an installed package (it lives inside `@effect/tsgo`), hence the `ignoreDependencies` entry in [knip.json](knip.json). The oxlint patch is version-coupled, so bump `@effect/tsgo`, `oxlint` and `oxlint-tsgolint` together and re-run `pnpm install`. Standalone LS run: `pnpm exec effect-tsgo diagnostics --project tsconfig.json`.
