...
**Call a new Pages REST endpoint**: whitelist + codegen (above), then in an `Effect.fn`: `const cloudflare = yield* CloudflareApi` and `yield* Effect.tryPromise({try: async () => unwrap(await cloudflare.GET('/accounts/{account_id}/...', {params: {path, query}})), catch: CloudflareApiError.from})` — `unwrap()` returns the typed `result` and throws on failure; `unwrapSuccess()` returns a boolean. Auth is injected once by the layer's client middleware — never set headers per-call. Mock with `interceptCloudflare(path, response, status, method)` ([`__tests__/helpers/api.ts`](__tests__/helpers/api.ts)).
