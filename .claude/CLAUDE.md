...
- **Line anchors**: some links here use line numbers (`create.ts#L100-L123`, `esbuild.config.js#L47-L63`) — update them when editing that code.
...
- Lint suppressions you'll need: `unicorn/no-useless-undefined` on `Config.withDefault(undefined)`, `unicorn/no-array-for-each` on `Effect.forEach`. Write `effect.pipe(Effect.map(f))`, not `Effect.map(effect, f)` — `unicorn/no-array-callback-reference` misreads the data-first form as `Array#map`.
