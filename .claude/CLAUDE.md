...
- **ESBuild** ([esbuild.config.js](esbuild.config.js)): banner adds a `createRequire` shim ([L22-L38](esbuild.config.js#L22-L38)); `wrangler` external; minification is syntax + whitespace only (`keepNames`, identifiers preserved); sourcemaps on.
...
- **Line anchors**: some links here use line numbers (`create.ts#L69-L88`, `esbuild.config.js#L22-L38`) — update them when editing that code.
...
- **Entry points** ([src/deploy/index.ts](src/deploy/index.ts)): `Effect.runPromiseExit(run)` then `setFailed(Cause.pretty(exit.cause))` unless interrupt-only. Never `void run()`/`runFork` (a failed deploy exits 0) or `NodeRuntime.runMain` (its `process.exit` can truncate buffered workflow commands on stdout).
...
- **Secrets**: `Config.redacted`; unwrap with `secret()` from [src/common/inputs.ts](src/common/inputs.ts) only at the point of use (auth header, child env).
