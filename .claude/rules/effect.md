...
- Prefix messages with a module-level `PREFIX` (e.g. `delete -`, `GitHub Environment:`) so annotations are attributable.
...
- Decode without throwing: `Schema.decodeUnknownOption(S)(x)` (`src/common/github/deployment/payload.ts`) or `Schema.decodeUnknownResult(S)(x)` when the failure message is wanted (`src/common/github/workflow-event/workflow-event.ts`); parse JSON text with `parseJson()` (`src/common/json.ts`), which yields `undefined` when invalid. Payload and event types derive from their schemas (`src/common/github/deployment/types.ts`, `src/common/github/workflow-event/types.ts`) — change the schema, not a hand-written type.
- Tunables that production never overrides (the poll interval and ceilings, the page cap, the wrangler timeout) are `Context.Reference`s with a default (`PollInterval`, `PollTimeout`, `PollCountMax` in `src/common/cloudflare/deployment/status.ts`; `PageCountMax` in `src/common/github/api/paginate.ts`; `WranglerTimeout` in `src/common/cloudflare/deployment/wrangler.ts`), so tests provide `Layer.succeed(PollInterval, Duration.zero)` instead of threading options through call sites.
- Poll (`src/common/cloudflare/deployment/status.ts`) with `Effect.retry` (`Schedule.spaced` + `Schedule.upTo({duration, times})`; `while` stays a plain boolean predicate) inside an outer `Effect.timeout` (the real ceiling), then `Effect.catchTag(['DeploymentPendingError', 'TimeoutError'], …)` → `DeploymentPollTimeoutError`. Don't retry `CloudflareApiError`.
