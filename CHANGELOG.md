...
- [#874](https://github.com/andykenward/github-actions-cloudflare-pages/pull/874) [`488ac73`](https://github.com/andykenward/github-actions-cloudflare-pages/commit/488ac7341b8946ce410c3154e95f0bbf0ecf214c) Thanks [@andykenward](https://github.com/andykenward)! - fix: a Cloudflare request that is interrupted or times out now aborts the underlying fetch, and every Cloudflare failure message names the request: a `success: false` envelope without errors reports `A request to the Cloudflare API (<url>) failed.` instead of `Cloudflare Delete Deployment: fail`, an empty 2xx reply reports its status (`failed: 204 No Content`), and a missing `result` reports `failed: response missing 'result'`.

- [#875](https://github.com/andykenward/github-actions-cloudflare-pages/pull/875) [`5d41f36`](https://github.com/andykenward/github-actions-cloudflare-pages/commit/5d41f36e58a937c2926f93d2f31111bc38386383) Thanks [@andykenward](https://github.com/andykenward)! - fix: the pull request comment puts Wrangler's output in a code block, so nothing in it is rendered as Markdown; interrupting or timing out a GitHub request now aborts it; the delete action's warnings say whether GitHub ran none of its mutations or only a later one failed.
...
- [#878](https://github.com/andykenward/github-actions-cloudflare-pages/pull/878) [`9eb0336`](https://github.com/andykenward/github-actions-cloudflare-pages/commit/9eb03368122c56da9d874502866979878373d3ba) Thanks [@andykenward](https://github.com/andykenward)! - List deployments with a plain `fetch` instead of Octokit, dropping the `@octokit-next/core` and `@octokit/plugin-paginate-rest` dependencies from the delete action. The listing now honours `GITHUB_API_URL`, so it works on GitHub Enterprise Server.
